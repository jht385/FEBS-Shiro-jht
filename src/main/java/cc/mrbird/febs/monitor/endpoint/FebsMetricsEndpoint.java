...
public class FebsMetricsEndpoint {
	...
	private Collection<Meter> findFirstMatchingMeters(CompositeMeterRegistry composite, String name,
			Iterable<Tag> tags) {
		return composite.getRegistries().stream().map((registry) -> this.findFirstMatchingMeters(registry, name, tags))
	...
	private void mergeMeasurements(Map<Statistic, Double> samples, Meter meter) {
		meter.measure().forEach((measurement) -> {
			samples.merge(measurement.getStatistic(), measurement.getValue(),
